...
func (ct *CollectionTools) handleExport(ctx context.Context, args map[string]interface{}) (interface{}, error) {
	...
	record, exists := ct.installer.GetInstalled(id)
	if !exists {
		return nil, fmt.Errorf("collection %s is not installed", id)
	}

	// Export to tar.gz
	if err := sources.ExportToTarGz(record.InstallLocation, output); err != nil {
