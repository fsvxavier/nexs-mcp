...
func (g *GPGVerifier) Verify(filePath string, signaturePath string, publicKey string) error {
	...
	if err != nil {
		return fmt.Errorf("signature verification failed: %s (output: %s)", err, string(output))
	}
...
func (g *GPGVerifier) importPublicKey(publicKey string) error {
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("key import failed: %s (output: %s)", err, string(output))
	}
	return nil
}
...
func (s *SSHVerifier) Verify(filePath string, signaturePath string, publicKeyPath string) error {
	if publicKeyPath == "" {
		return fmt.Errorf("public key path is required for SSH verification")
	}
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("SSH signature verification failed: %s (output: %s)", err, string(output))
	}
...
func (m *SignatureManager) VerifyAny(filePath string, signaturePath string, publicKey string) error {
	if len(m.verifiers) == 0 {
		return fmt.Errorf("no verifiers registered")
	}
...
func (m *SignatureManager) InitializeDefaultVerifiers() error {
	...
	if len(m.verifiers) == 0 {
		return fmt.Errorf("no signature verifiers available (install gpg or ssh-keygen)")
	}
